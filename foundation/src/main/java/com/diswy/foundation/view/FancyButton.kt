...
import com.diswy.foundation.config.Constants
...
class FancyButton @JvmOverloads constructor(
    ...
    private var mDisableColor: Int = Constants.DEFAULT_DISABLE// 默认不可用颜色
    ...
    init {
        if (attrs != null) {
            ...
            mBorderColor =
                a.getInteger(R.styleable.FancyButton_fb_border_color, Constants.DEFAULT_BORDER)
            mSolidNormal =
                a.getInteger(R.styleable.FancyButton_fb_solid_normal, Constants.DEFAULT_TRANSPARENT)
            mSolidPressed =
                a.getInteger(R.styleable.FancyButton_fb_solid_pressed, Constants.DEFAULT_PRESSED)
            mCornerRadius =
                a.getDimension(R.styleable.FancyButton_fb_corner_radius, Constants.CORNER_RADIUS)
            mDisableColor =
                a.getInteger(R.styleable.FancyButton_fb_disable_color, Constants.DEFAULT_DISABLE)
            ...
            mDrawable = builder.strokeWidth(mBorderWidth.toInt())
                ...
                .solidColorDisabled(mDisableColor)
                .ripple(mRipple)
                .build()
        }
    }
