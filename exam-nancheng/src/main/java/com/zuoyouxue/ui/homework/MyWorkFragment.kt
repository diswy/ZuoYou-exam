...
import com.zuoyouxue.databinding.FragmentMyWorkBinding
import kotlinx.android.synthetic.main.activity_my_work.*


/**
 * 我的作业页，核心业务,同activity逻辑
 */
class MyWorkFragment : BaseBindFragment<FragmentMyWorkBinding>(),
    ...
    override fun getLayoutRes(): Int = R.layout.fragment_my_work
    ...
    private fun initControl(selected: Int) {
        when (selected) {
            ControlDrawableSpec.LEFT -> {
                ...
                binding.unfinished.setTextColor(
                    ContextCompat.getColor(
                        requireContext(),
                        R.color.title
                    )
                )
                binding.finished.setTextColor(
                    ContextCompat.getColor(
                        requireContext(),
                        R.color.white
                    )
        ...
        binding.unfinished.background =
            ControlDrawableSpec.build(ControlDrawableSpec.LEFT, requireContext())
        binding.finished.background =
            ControlDrawableSpec.build(ControlDrawableSpec.RIGHT, requireContext())
    }
